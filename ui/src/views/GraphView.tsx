 ...
 *
 * Layout: dagre Sugiyama (top-to-bottom), snapped to architectural layer bands.
 * Stable (core) layer sits at the bottom; unstable (ui) at the top.
 * Healthy imports flow downward; violations and upward arrows are visually jarring.
 */
...
import { layoutWithDagre, NODE_H } from './graph/layout'
import {
  ModuleNode, layerColor, LAYER_COLORS,
  type ModuleSignals, type ModuleNodeData,
} from './graph/ModuleNode'
...
function computeSignals(
  ...
  // Hub threshold: top 10% by fan-in (min fan_in of 2 to avoid flagging imports in tiny repos)
  const fanIns = model.modules
    .map(m => model.metrics[m.id]?.fan_in ?? 0)
    .sort((a, b) => a - b)
  const p90 = fanIns[Math.floor(fanIns.length * 0.9)] ?? 1
  const hubThreshold = Math.max(2, p90)

  const violSrcPaths = new Set(
    violations.filter(v => v.severity === 'Error').map(v => v.from_path),
  )

  const dupePaths = new Set<string>()
  if (dupes) {
    for (const c of dupes.clusters) {
      for (const f of c.files) dupePaths.add(f)
    }
  }
  ...
  for (const m of model.modules) {
    ...
    const fi = met?.fan_in ?? 0
    const fo = met?.fan_out ?? 0
    result[m.id] = {
      ...
      changeStatus: addedPaths.has(m.path)   ? 'added'
                  : removedPaths.has(m.path) ? 'removed'
                  : null,
      loc: met?.loc ?? 0,
      layer: layers[m.id],
    }
...
// ─── helper to build node width from LOC ─────────────────────────────────────

function nodeWidth(loc: number): number {
  // Clamp width: 148 (base) → 224 (max for ~400+ LOC files)
...
function GraphViewInner() {
  ...
  const visibleModules = useMemo(() => {
    ...
    // Focus mode: only the selected node + its 1-hop neighbourhood
    if (focusId !== null) {
    ...
    if (search) {
      ...
      mods = mods.filter(m => m.path.toLowerCase().includes(q))
    }
  ...
  // ── build React Flow nodes + edges, run layout ───────────────────────────
  useEffect(() => {
    if (!model) return

    const visibleIds = new Set(visibleModules.map(m => m.id))

    // Build violation edge key set
    const violEdgeKeys = new Set(violations.map(v => `${v.from_path}|${v.to_path}`))

    // Nodes
    const rawNodes: FlowNode[] = visibleModules.map(m => {
      const metrics  = model.metrics[m.id] ?? { fan_in: 0, fan_out: 0, coupling: 0 }
      const signals  = signalsMap[m.id] ?? DEFAULT_SIGNALS
      const w        = nodeWidth(signals.loc)
      const isSelected = m.id === selectedId
      const isDimmed   = neighborhood !== null && !neighborhood.has(m.id)
    ...
    // Edges
    const rawEdges: FlowEdge[] = model.edges
      ...
      .map((e, i) => {
        ...
        let stroke = e.kind === 'Reexports' ? '#fbbf24' : '#4338ca'
        if (isViol)      stroke = '#f87171'
        else if (isCycleEdge) stroke = '#ef4444'
        ...
        return {
          ...
          animated: e.kind === 'Reexports' && !isViol && !isCycleEdge,
          markerEnd: { type: MarkerType.ArrowClosed, color: stroke },
        }
      })

    // Layout
    const layerIdxOf = (nodeId: string): number => {
      const m = model.modules.find(m => String(m.id) === nodeId)
      if (!m) return -1
      const layer = signalsMap[m.id]?.layer
      if (!layer) return -1
      return layerOrder.indexOf(layer)
    }

    const laidOut = layoutWithDagre(rawNodes, rawEdges, layerIdxOf, layerOrder.length)

    setNodes(laidOut)
    setEdges(rawEdges)
    setFitGeneration(g => g + 1)
  }, [model, visibleModules, signalsMap, violations, selectedId, neighborhood, layerOrder])

  // ── fit view after layout change ─────────────────────────────────────────
  useEffect(() => {
    if (fitGeneration > 0) {
      // Small delay lets React Flow measure the new node dimensions
      const id = setTimeout(() => fitView({ padding: 0.12, duration: 300 }), 60)
      return () => clearTimeout(id)
    }
  }, [fitGeneration, fitView])
      ...
      >
        ...
        <MiniMap
          nodeColor={n => {
            ...
            const sig = signalsMap[id]
            if (!sig) return '#4338ca'
            if (sig.isCycle) return '#f87171'
            return layerColor(sig.layer)
          }}
          ...
          {focusId !== null && (
            <button
              ...
              style={{
                ...
                color: 'var(--warning)', cursor: 'pointer', display: 'flex',
        ...
        {/* ── stats card (bottom-left above minimap) ────────────────────── */}
        {model && (
          <div style={{
            position: 'absolute', bottom: 130, left: 12, zIndex: 10,
            background: 'var(--surface)', border: '1px solid var(--border)',
...
function DetailPanel({
  ...
  return (
    <div style={{
      ...
      {/* violations affecting this module */}
      {violations.length > 0 && (
          ...
          {violations.map((v, i) => (
            <div
              ...
              style={{
                ...
                borderRadius: 5, padding: '5px 8px', marginBottom: 5, fontSize: 11,
              }}
            >
